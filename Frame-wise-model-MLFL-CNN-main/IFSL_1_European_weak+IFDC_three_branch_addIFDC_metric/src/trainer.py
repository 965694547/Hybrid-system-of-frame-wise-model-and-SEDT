...
from src import model_PT as md_PT
...
class trainer(object):
	def __init__(self, task_name, model_name, is_PS, from_exp, win_conf=None):
		""""
		...
		if from_exp:
			self.conf_dir = os.path.join('exp_sedm', task_name, model_name, 'conf')
	...
	def init_model(self):
		...
		if(is_PS == True):
			...
			self.model_struct = md_PT.attend_cnn(conf_dir, model_name, task_name, 
				data_loader.LEN, data_loader.DIM, data_loader.CLASS)
	...
	def init_utils(self, win_conf):
		...
		self.utils = utils.utils(conf_dir, exp_dir, data_loader.events, win_conf)
		lst, csv, dur_csv = data_loader.get_test()
		self.utils.init_csv(csv)
		lst, csv, dur_csv = data_loader.get_vali()
		self.utils.init_csv(csv)
		self.utils.set_vali_csv(lst, csv, dur_csv)
	...
	def prepare_exp(self):
		...
		if not os.path.exists('exp_sedm'):
			os.mkdir('exp_sedm')

		#If task dir in the experimental dir doesn't exist, then create
		root_dir = os.path.join('exp_sedm', task_name)
	...
	def test(self, data_set, mode, preds = {}):
		...
		if mode == 'at':
			...
			label = data[1][0][:, :data_loader.CLASS]
			
			preds = preds[:, :data_loader.CLASS]
			...
			at_pred = preds[0][:, :data_loader.CLASS]
			sed_pred = preds[1]
	...
	def save_at(self, mode = 'test', at_preds = {}, is_add = False):
		...
		f1_utils.set_vali_csv(lst, csv, dur_csv)
	...
	def save_psds_result(self, sed_preds={}):
		""""
                Predict and save event detection performance both on validation set
		and test set.
		Args:
		...
		"""
		preds_out = {}
		#preds_out['vali'] = self.save_psds(mode='vali',
		#								  sed_preds=sed_preds, is_add=False)
		preds_out['test'] = self.save_psds(mode='test',
										  sed_preds=sed_preds, is_add=True)
		return preds_out

	def save_psds(self, mode='test', sed_preds={}, is_add=False):
		""""
		Args:
			mode: string in ['vali','test']
				the dataset to predict
		...
		data_loader = self.data_loader
		psds_utils = self.utils

		result_path = os.path.join(result_dir, model_name + '_psds.txt')
		...
		o = '[ result sed %s psds_score_scenario1: %f, psds_score_scenario2 : %f ]' % (mode, psds_1, psds_2)

		# show result
		LOG.info(o)

		# save result
		self.save_str(result_path, [o], is_add)
	...
	def chunk_process(self, start, end, psds_utils, preds, frame_preds, mode):
		win_len_out_1 = list()
		win_len_out_2 = list()
		for win_len_i in tqdm(range(start, end)):
			psds_utils.tune_win_lens(win_len_i)
			# get psds
			psds_1, psds_1_per_class, psds_2, psds_2_per_class = psds_utils.get_psds_tune(preds, frame_preds, mode=mode)
			o = '[ result sed %s psds_score_scenario1: %f, psds_score_scenario2 : %f ]' % ('tune', psds_1, psds_2)
			# show result
			LOG.info(o)
			psds_1_per_class.append(psds_1)
			psds_2_per_class.append(psds_2)
			win_len_out_1.append(psds_1_per_class)
			win_len_out_2.append(psds_2_per_class)
		return win_len_out_1, win_len_out_2

	def tune_psds(self, mode='test', sed_preds={}):
		""""
		Args:
			mode: string in ['vali','test']
				the dataset to predict
		...
		p = Pool(os.cpu_count())
		start2end = self.win_len_max // os.cpu_count()
		futures = []
		for start in tqdm(range(1, self.win_len_max + 1, start2end)):
			end = start + start2end
			if end > self.win_len_max + 1:
				end = self.win_len_max + 1
			futures.append(p.apply_async(self.chunk_process, args=[start, end, psds_utils, preds, frame_preds, mode]))
		p.close()
		for f in tqdm(futures):
			win_len_out_1_, win_len_out_2_ = f.get()
			win_len_out_1.extend(win_len_out_1_)
			win_len_out_2.extend(win_len_out_2_)
		p.join()

		pd.DataFrame(win_len_out_1).to_csv(os.path.join(result_dir, 'psds_1_win.csv'))
		pd.DataFrame(win_len_out_2).to_csv(os.path.join(result_dir, 'psds_2_win.csv'))
		preds = np.reshape(preds, [preds.shape[0], 1, preds.shape[1]])

		# return frame-level prediction (probilities)
		return ori_frame_preds * preds

	def pro_sedt(self, featurenames, mode='sed', preds = {}):
		data = list()
		conf_dir = self.conf_dir
		...
		#pdb.set_trace()
		feature_dir = config['path']['feature_dir']
		for featurename in featurenames:
			data_f = os.path.join(feature_dir, featurename + '.npy')
			assert os.path.exists(data_f)
			data.append(np.load(data_f))
		data = np.array(data)
		...
		CLASS = data_loader.CLASS
		# predict
		model = self.model_struct.get_model(
			pre_model=best_model_path,
			mode=mode)
		preds = model.predict(data, batch_size=batch_size)
		at_pred = preds[0][:, :CLASS]
		sed_pred = preds[1]
		return at_pred, sed_pred

	def post_sedt(self, lst, preds, frame_preds, gt_df, pred_dfs, at_m, test_n_thresholds=50, SEDT_psds_prediction_dfs_0=None, vote=None):
		psds_utils = self.utils
		return psds_utils.psds_sedt(lst, preds, frame_preds, gt_df, pred_dfs, at_m, test_n_thresholds=test_n_thresholds, SEDT_psds_prediction_dfs_0=SEDT_psds_prediction_dfs_0, vote=vote)
