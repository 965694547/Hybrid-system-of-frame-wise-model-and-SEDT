			...
			if weak:
				...
				if(sep_str == '_' or sep_str == 'sep' or sep_str == 'not_sep'):
					...
					for i in range(2):
						...
						losses += [K.concatenate([closs] + uloss, axis = 0)]
					culoss = losses[0]
				...
				syn_w = np.load('mask_weights/syn-1_weights.npy')
				...
				for j in range(1, M):
					mask_1 = masks[j - 1]
					uloss += [2.0 * b * K.mean(mask_1 * K.mean(K.binary_crossentropy(y_true[j::M], y_pred[j::M, :, : CLASS]), 
																									axis = 1), axis = -1)]
